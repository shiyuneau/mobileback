/**
  * Copyright 2019 bejson.com 
  */
package com.sy.mobileback.accessdb.domain;
import java.util.List;

/**
 * Auto-generated: 2019-04-01 13:41:50
 *
 * @author bejson.com (i@bejson.com)
 * @website http://www.bejson.com/java2pojo/
 */
public class LXApplyEntity {

    private String userid;
    private String passportcode;
    private String passportexpire;
    private String familyname;
    private String givenname;
    private String highestdegree;
    private String religion;
    private String nationality;
    private String mothertongue;
    private boolean emigrantfrom;
    private String photoid;
    private int sex;
    private String birth;
    private String birthplace;
    private String maritalstatus;
    private String college;
    private String taughtby;
    private String startyear;
    private String fundingsource;
    private String studyfield;
    private String degree;
    private String admissionseason;
    private String english;
    private String ielts;
    private String hsk;
    private String toefl;
    private String referencetel;
    private String finacial;
    private String guarantorin;
    private String guardian;
    private List<EducationList> educationList;
    private List<WorkList> workList;
    private long validpassport;
    private long graduatecertificate;
    private long highestacademic;
    private long studyplan;
    private long medicalrepott;
    private long chineselanguagecertificate;
    private long juniorcollegedipint;
    private long collegetranscript;
    private String streetadress;
    private String cityprovince;
    private String country;
    private String countryemail;
    private String countryphome;
    private String countryzipcode;
    private String applyemail;
    private String applyphone;
    private String applyaddress;
    private String applyzipcode;
    private String receivername;
    private String receivercity;
    private String receiveraddress;
    private String receiverphone;
    private String receivercounty;
    private String receiverzipcode;
    private List<FamilyList> familyList;
    private String applicationnode;
    public void setUserid(String userid) {
         this.userid = userid;
     }
     public String getUserid() {
         return userid;
     }

    public void setPassportcode(String passportcode) {
         this.passportcode = passportcode;
     }
     public String getPassportcode() {
         return passportcode;
     }

    public void setPassportexpire(String passportexpire) {
         this.passportexpire = passportexpire;
     }
     public String getPassportexpire() {
         return passportexpire;
     }

    public void setFamilyname(String familyname) {
         this.familyname = familyname;
     }
     public String getFamilyname() {
         return familyname;
     }

    public void setGivenname(String givenname) {
         this.givenname = givenname;
     }
     public String getGivenname() {
         return givenname;
     }

    public void setHighestdegree(String highestdegree) {
         this.highestdegree = highestdegree;
     }
     public String getHighestdegree() {
         return highestdegree;
     }

    public void setReligion(String religion) {
         this.religion = religion;
     }
     public String getReligion() {
         return religion;
     }

    public void setNationality(String nationality) {
         this.nationality = nationality;
     }
     public String getNationality() {
         return nationality;
     }

    public void setMothertongue(String mothertongue) {
         this.mothertongue = mothertongue;
     }
     public String getMothertongue() {
         return mothertongue;
     }

    public void setEmigrantfrom(boolean emigrantfrom) {
         this.emigrantfrom = emigrantfrom;
     }
     public boolean getEmigrantfrom() {
         return emigrantfrom;
     }

    public void setPhotoid(String photoid) {
         this.photoid = photoid;
     }
     public String getPhotoid() {
         return photoid;
     }

    public void setSex(int sex) {
         this.sex = sex;
     }
     public int getSex() {
         return sex;
     }

    public void setBirth(String birth) {
         this.birth = birth;
     }
     public String getBirth() {
         return birth;
     }

    public void setBirthplace(String birthplace) {
         this.birthplace = birthplace;
     }
     public String getBirthplace() {
         return birthplace;
     }

    public void setMaritalstatus(String maritalstatus) {
         this.maritalstatus = maritalstatus;
     }
     public String getMaritalstatus() {
         return maritalstatus;
     }

    public void setCollege(String college) {
         this.college = college;
     }
     public String getCollege() {
         return college;
     }

    public void setTaughtby(String taughtby) {
         this.taughtby = taughtby;
     }
     public String getTaughtby() {
         return taughtby;
     }

    public void setStartyear(String startyear) {
         this.startyear = startyear;
     }
     public String getStartyear() {
         return startyear;
     }

    public void setFundingsource(String fundingsource) {
         this.fundingsource = fundingsource;
     }
     public String getFundingsource() {
         return fundingsource;
     }

    public void setStudyfield(String studyfield) {
         this.studyfield = studyfield;
     }
     public String getStudyfield() {
         return studyfield;
     }

    public void setDegree(String degree) {
         this.degree = degree;
     }
     public String getDegree() {
         return degree;
     }

    public void setAdmissionseason(String admissionseason) {
         this.admissionseason = admissionseason;
     }
     public String getAdmissionseason() {
         return admissionseason;
     }

    public void setEnglish(String english) {
         this.english = english;
     }
     public String getEnglish() {
         return english;
     }

    public void setIelts(String ielts) {
         this.ielts = ielts;
     }
     public String getIelts() {
         return ielts;
     }

    public void setHsk(String hsk) {
         this.hsk = hsk;
     }
     public String getHsk() {
         return hsk;
     }

    public void setToefl(String toefl) {
         this.toefl = toefl;
     }
     public String getToefl() {
         return toefl;
     }

    public void setReferencetel(String referencetel) {
         this.referencetel = referencetel;
     }
     public String getReferencetel() {
         return referencetel;
     }

    public void setFinacial(String finacial) {
         this.finacial = finacial;
     }
     public String getFinacial() {
         return finacial;
     }

    public void setGuarantorin(String guarantorin) {
         this.guarantorin = guarantorin;
     }
     public String getGuarantorin() {
         return guarantorin;
     }

    public void setGuardian(String guardian) {
         this.guardian = guardian;
     }
     public String getGuardian() {
         return guardian;
     }

    public void setEducationList(List<EducationList> educationList) {
         this.educationList = educationList;
     }
     public List<EducationList> getEducationList() {
         return educationList;
     }

    public void setWorkList(List<WorkList> workList) {
         this.workList = workList;
     }
     public List<WorkList> getWorkList() {
         return workList;
     }

    public void setValidpassport(long validpassport) {
         this.validpassport = validpassport;
     }
     public long getValidpassport() {
         return validpassport;
     }

    public void setGraduatecertificate(long graduatecertificate) {
         this.graduatecertificate = graduatecertificate;
     }
     public long getGraduatecertificate() {
         return graduatecertificate;
     }

    public void setHighestacademic(long highestacademic) {
         this.highestacademic = highestacademic;
     }
     public long getHighestacademic() {
         return highestacademic;
     }

    public void setStudyplan(long studyplan) {
         this.studyplan = studyplan;
     }
     public long getStudyplan() {
         return studyplan;
     }

    public void setMedicalrepott(long medicalrepott) {
         this.medicalrepott = medicalrepott;
     }
     public long getMedicalrepott() {
         return medicalrepott;
     }

    public void setChineselanguagecertificate(long chineselanguagecertificate) {
         this.chineselanguagecertificate = chineselanguagecertificate;
     }
     public long getChineselanguagecertificate() {
         return chineselanguagecertificate;
     }

    public void setJuniorcollegedipint(long juniorcollegedipint) {
         this.juniorcollegedipint = juniorcollegedipint;
     }
     public long getJuniorcollegedipint() {
         return juniorcollegedipint;
     }

    public void setCollegetranscript(long collegetranscript) {
         this.collegetranscript = collegetranscript;
     }
     public long getCollegetranscript() {
         return collegetranscript;
     }

    public void setStreetadress(String streetadress) {
         this.streetadress = streetadress;
     }
     public String getStreetadress() {
         return streetadress;
     }

    public void setCityprovince(String cityprovince) {
         this.cityprovince = cityprovince;
     }
     public String getCityprovince() {
         return cityprovince;
     }

    public void setCountry(String country) {
         this.country = country;
     }
     public String getCountry() {
         return country;
     }

    public void setCountryemail(String countryemail) {
         this.countryemail = countryemail;
     }
     public String getCountryemail() {
         return countryemail;
     }

    public void setCountryphome(String countryphome) {
         this.countryphome = countryphome;
     }
     public String getCountryphome() {
         return countryphome;
     }

    public void setCountryzipcode(String countryzipcode) {
         this.countryzipcode = countryzipcode;
     }
     public String getCountryzipcode() {
         return countryzipcode;
     }

    public void setApplyemail(String applyemail) {
         this.applyemail = applyemail;
     }
     public String getApplyemail() {
         return applyemail;
     }

    public void setApplyphone(String applyphone) {
         this.applyphone = applyphone;
     }
     public String getApplyphone() {
         return applyphone;
     }

    public void setApplyaddress(String applyaddress) {
         this.applyaddress = applyaddress;
     }
     public String getApplyaddress() {
         return applyaddress;
     }

    public void setApplyzipcode(String applyzipcode) {
         this.applyzipcode = applyzipcode;
     }
     public String getApplyzipcode() {
         return applyzipcode;
     }

    public void setReceivername(String receivername) {
         this.receivername = receivername;
     }
     public String getReceivername() {
         return receivername;
     }

    public void setReceivercity(String receivercity) {
         this.receivercity = receivercity;
     }
     public String getReceivercity() {
         return receivercity;
     }

    public void setReceiveraddress(String receiveraddress) {
         this.receiveraddress = receiveraddress;
     }
     public String getReceiveraddress() {
         return receiveraddress;
     }

    public void setReceiverphone(String receiverphone) {
         this.receiverphone = receiverphone;
     }
     public String getReceiverphone() {
         return receiverphone;
     }

    public void setReceivercounty(String receivercounty) {
         this.receivercounty = receivercounty;
     }
     public String getReceivercounty() {
         return receivercounty;
     }

    public void setReceiverzipcode(String receiverzipcode) {
         this.receiverzipcode = receiverzipcode;
     }
     public String getReceiverzipcode() {
         return receiverzipcode;
     }

    public void setFamilyList(List<FamilyList> familyList) {
         this.familyList = familyList;
     }
     public List<FamilyList> getFamilyList() {
         return familyList;
     }

    public void setApplicationnode(String applicationnode) {
         this.applicationnode = applicationnode;
     }
     public String getApplicationnode() {
         return applicationnode;
     }

}